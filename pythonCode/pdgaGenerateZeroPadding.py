...
def generate_mcplayer(dDir):
    for i in range (4, 2*bf.maxAmbiOrder+2):
        ind = i
        #opens a Pure Data file for the mc.regdecoder#ind.pd abstraction
        fileName = dDir+'/mc.zeropadding'+str(ind)+".pd"
        f = open(fileName, 'w')
        #writes the lines of the mc.player#ind.pd Pure Data abstraction
        #writes the objects
        f.write(bf.patchMiddleCanvas)
        f.write(bf.patchAbstractionCnv1_1+' mc.zeropadding'+str(ind)+' '+bf.patchAbstractionCnv2_1+'\n')
        bf.resetObjInd()
        ...
        in_id = bf.appendXObj(f, 1, 0, 'inlet~')
        #snake~ out 2 object to decompose the 2 channels
        snakeout_id = bf.appendXObj(f, 2, 0, 'snake~ out 2')
        #openpanel object
